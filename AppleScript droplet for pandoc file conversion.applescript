...
on open dropped_files
	...
	try
		...
		set inputFormats to {"bibtex", "biblatex", "commonmark", "commonmark_x", "creole", "csljson", "csv", "docbook", "docx", "dokuwiki", "endnotexml", "epub", "fb2", "gfm", "haddock", "html", "ipynb", "jats", "jira", "json", "latex", "man", "markdown", "markdown_github", "markdown_mmd", "markdown_phpextra", "markdown_strict", "mediawiki", "muse", "native", "odt", "opml", "org", "ris", "rst", "rtf", "t2t", "textile", "tikiwiki", "twiki", "typst", "vimwiki"}
		set inputDialogResult to {choose from list inputFormats with title "Pandoc: Specify input format" with prompt "What is the format of the file(s) to be converted? (Note that markdown_github is deprecated in favor of gfm for GitHub-Flavored Markdown.)" default items "markdown"}
		...
		repeat with i in dropped_files
			...
			if the folder of file_info is false then
				...
				repeat with o from 1 to the count of output_format_words
					...
					if output_format is "chunkedhtml" then
						set output_extension to "-chunkedhtml.zip"
					end if
					...
					if output_format is "typst" then
						set output_extension to ".typ"
					end if
